...
export class PerspectiveProviderComponent {
  ...
  // reset the form
  clearForm() {
    this.myForm.controls['providerName'].reset();
    this.myForm.controls['contactPersonName'].reset();
    this.myForm.controls['contactEmailID'].reset();
  ...
  onSubmit(): void {
    ...
    if (this.myForm.invalid) {
      this.toastrService.info(
        'All required values should be provided!',
        'Required',
        {
          closeButton: true,
        }
  ...
  loadData() {
    ...
    this.commonService
      .getMethodWithParams(url, { parent_ticket_id: this.PTicket_id })
      .subscribe(
        (res: any) => {
          console.log(res);
